const imgprocess = new ImgProcess();
...
async function searchCommand(command) {
    if (command.startsWith("/")) {
        ...
        if (!comandoEncontrado) {
            ...
            if (comanaddtext) {
                await _ajax("/save-cog", "POST", { [comanaddtext.comando]: valor === "false" ? false : valor });
            }
            return true;
        }
...
function addNewItems(items) {
    ...
    for (const item of items) {
        const newItem = saved.getSaved("template")(item);
    ...
    // Start Saving in cache
    setTimeout(async () => {
        let { cache = "on" } = saved.getSaved("cog");
        if (cache == "on") {
            const datasave = saved.getSaved("searchText");
            await _ajax("/start-process", "POST", { name: datasave.searchText + "_" + datasave.page });
            loadImg();
        } else {
            // Hide Loading
...
async function loadImg() {
    const $artAll = $("#art-all");
    const $existingItems = $artAll.children();
    $existingItems.each(async function (index) {
        const $item = $(this);
        const slug = $item.attr("data-slug");
        const bg = $item.attr("data-bg");
        let imglist = await _ajax("/load-img", "POST", { slug, bg });
        // set img
        $(`#id_${slug}`).css("background-image", `url('${imglist}')`);
...
kit.onDOMReady(async () => {
    // cog
    let { thekey, modalkey = "on" } = await fet("/cog");
    if (!thekey) {
        if (modalkey == "on") {
            kit.modal("materialize", "#thekey");
        }
    ...
    $(".no-show-api").on("click", async () => {
        await _ajax("/save-cog", "POST", { modalkey: "off" });
    })
...
// Template
saved.addSaved("template", (item) => {
    let { cache = "on" } = saved.getSaved("cog");
    if (cache === "on") {
        if (item.background_image) {
            imgprocess.loadImg(item.background_image, item.slug);
        }
    }
